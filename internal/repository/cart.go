...
func (r *CartRepository) AddToCart(ctx context.Context, cart *models.Cart) error {
	...
	checkQuery := `
		SELECT id FROM cart
		WHERE customer_id = $1 AND product_id = $2 AND size_id IS NOT DISTINCT FROM $3 AND temperature_id IS NOT DISTINCT FROM $4
	`

	err := r.db.QueryRow(ctx, checkQuery, cart.CustomerID, cart.ProductID, cart.SizeID, cart.TemperatureID).Scan(&existingID)

	if err == nil {
		return r.UpdateCartQuantity(ctx, existingID, cart.Quantity, true)
	} else if err == pgx.ErrNoRows {
		query := `
			INSERT INTO cart (customer_id, product_id, size_id, temperature_id, quantity)
			VALUES ($1, $2, $3, $4, $5)
		`
...
func (r *CartRepository) GetCartItemByID(ctx context.Context, cartItemID int) (*models.Cart, error) {
	...
	query := `
		SELECT id, customer_id, product_id, size_id, temperature_id, quantity, created_at
		FROM cart
		WHERE id = $1
	`

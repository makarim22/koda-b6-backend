...
func (s *PaymentService) Create(ctx context.Context, payment *models.Payment) error {
	if payment.OrderID <= 0 {
		return errors.NewValidationError("order ID", "must be positive")
	}
	if payment.Amount <= 0 {
	...
	if !validMethods[payment.Method] {
		return errors.NewValidationError("payment_method", "invalid payment method")
	}
	...
	if err != nil {
		if errors.IsNotFoundError(err) {
			return errors.NewValidationError("order", "not found")
		}
...
func (s *PaymentService) GetByOrderID(ctx context.Context, orderID int) (*models.Payment, error) {
	if orderID <= 0 {
		return nil, errors.NewValidationError("order ID", "must be positive")
	}
...
func (s *PaymentService) UpdateStatus(ctx context.Context, id int, status string) error {
	if id <= 0 {
		return errors.NewValidationError("payment ID", "must be positive")
	}

	validStatuses := map[string]bool{"pending": true, "completed": true, "failed": true, "cancelled": true}
	if !validStatuses[status] {
		return errors.NewValidationError("payment_status", "invalid payment status")
	}
